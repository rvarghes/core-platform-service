/*
 * BookingSearchCommandHandler.java
...
/**
 * Command handler class for booking search
 * 
 * @author nvittal
 * @version
 * 
 ...
 * @since
 */
public class BookingSearchCommandHandler extends ABECommandHandlerBase {
	...
	public ABECommand handle(ABECommand command) {
		if (!(command instanceof BookingSearchCommand)) {
			String message = this.getMessageSource().getMessage(
					"ILLEAGAL_COMMAND_ERROR",
					new Object[] { "BookingSearchCommand",
		...
		BookingSearchCommand bookingSearchCommand = (BookingSearchCommand) command;
		BookingSearchResponse searchResponse = bookingSearchService
				.search(bookingSearchCommand.getBookingSearchCriteria());

		// Process response and set the business errors, warnings, and info
		// messages to command
		this.processResponse(bookingSearchCommand, searchResponse);
		bookingSearchCommand.setBookings(searchResponse.getBookings());
		return bookingSearchCommand;
	}
}