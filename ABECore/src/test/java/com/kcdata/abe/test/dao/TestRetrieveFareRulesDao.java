...
/**
 * UNIT test class to test Fare Rules DAO
 * 
 * @author nvittal
 * @version
 * 
...
public class TestRetrieveFareRulesDao {

	@Autowired
	private FareRulesDao fareRulesDao;

	/**
	 * Method to test the Fare Rules DAO for retrieving fare rules
	...
	public void retrieveFareRules() throws ParseException {
		...
		ABEApplicationContext applicationContext = this.fareRulesDao
				.getApplicationContextFactory().getApplicationContext();
		if (applicationContext == null) {
			applicationContext = this.fareRulesDao.getApplicationContextFactory()
					.create();
		}
		this.fareRulesDao.getApplicationContextFactory().getApplicationContext()
				.getRequestContext().setIMApplicationInfo(applicationInfo);

		FareRulesResponse fareRulesResponse = this.fareRulesDao
				.retrieveFareRules(getTripFlight(), false);
		assertNotNull(fareRulesResponse);
	}

	/**
	 * Method to test the Fare Rules DAO for retrieving additional fare rules
	 * @throws ParseException 
	 * 
	 * @see
	 * @since
	...
	public void retrieveAdditionalFareRules() throws ParseException {
		...
		FareRulesResponse fareRulesResponse = this.fareRulesDao
				.retrieveFareRules(getTripFlight(), true);
		assertNotNull(fareRulesResponse);
	}
